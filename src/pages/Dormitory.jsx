...
function Dormitory() {
  ...
  const [repairForm, setRepairForm] = useState({ building: '', room: '', type: '', description: '' })
  ...
  const buildings = [
    { name: '男生1号楼', rooms: 24, capacity: 96, occupied: 89, type: 'male', floor: '4层' },
    { name: '男生2号楼', rooms: 24, capacity: 96, occupied: 72, type: 'male', floor: '4层' },
    { name: '男生3号楼', rooms: 24, capacity: 96, occupied: 91, type: 'male', floor: '4层' },
    { name: '女生1号楼', rooms: 24, capacity: 96, occupied: 85, type: 'female', floor: '4层' },
    { name: '女生2号楼', rooms: 24, capacity: 96, occupied: 68, type: 'female', floor: '4层' },
    { name: '女生3号楼', rooms: 24, capacity: 96, occupied: 79, type: 'female', floor: '4层' },
  ]
  ...
  const stats = [
    { icon: Building2, label: '宿舍楼栋', value: '6栋', color: 'text-blue-600', bg: 'bg-blue-50' },
    { icon: BedDouble, label: '总床位', value: totalCapacity.toLocaleString(), color: 'text-purple-600', bg: 'bg-purple-50' },
    { icon: Shield, label: '入住率', value: `${overallRate}%`, color: 'text-emerald-600', bg: 'bg-emerald-50' },
  ]
    ...
    <div className="max-w-7xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {buildings.map((building) => (
          <div key={building.name} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 card-glow">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">{building.floor} · {building.rooms}间</p>
                </div>
              </div>
            </div>
            <div className="space-y-3">
              ...
              <div className="flex items-center gap-2 pt-2">
                <button
                  onClick={() => setShowAssignModal(building.name)}
                ...
                <button
                  onClick={() => setShowRepairModal(building.name)}
                  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-sm font-medium text-gray-600 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors glow-hover"
                >
                  <Wrench size={14} />
                  报修管理
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">报修申请 — {showRepairModal}</h3>
              <button onClick={() => setShowRepairModal(false)} className="p-1 hover:bg-gray-100 rounded-lg transition-colors">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input
                  type="text"
                  value={showRepairModal}
                  readOnly
                  className="w-full px-3 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-600"
                />
              </div>
              <div>
            ...
            <div className="p-6 border-t border-gray-100 flex items-center justify-end gap-3">
              ...
              <button
                onClick={() => {
                  ...
                  addToast('报修申请已提交，维修人员将在24小时内处理')
                }}
                className="px-4 py-2.5 text-sm font-medium text-white bg-primary-600 rounded-xl hover:bg-primary-700 transition-colors flex items-center gap-2 glow-accent"
              >
                <Save size={16} />
                提交报修
              </button>
      ...
      {showAssignModal && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-md overflow-hidden">
            <div className="p-6 border-b border-gray-100 flex items-center justify-between">
              <h3 className="text-lg font-semibold text-gray-900">房间分配 — {showAssignModal}</h3>
              <button onClick={() => setShowAssignModal(false)} className="p-1 hover:bg-gray-100 rounded-lg transition-colors">
                <X size={20} className="text-gray-500" />
              </button>
            </div>
            <div className="p-6 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                    {Array.from({ length: 4 }, (_, i) => <option key={i + 1}>第{i + 1}层</option>)}
                  </select>
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">房间号</label>
                  <select className="w-full px-3 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent">
                    {Array.from({ length: 6 }, (_, i) => <option key={i + 1}>{String(i + 1).padStart(2, '0')}室</option>)}
                  </select>
                </div>
              </div>
            </div>
